 ...
 * </UL>
 */
public class BasicCollection<E> extends java.util.AbstractCollection<E> implements Cloneable {
	
	private static final int DEFAULT_SIZE = 100;

	/**
	 * The array buffer into which the elements are stored. The array is resized
	 * when an add() occurs and the capacity of the array has been reached.
	 */
	 ...
	 * This information is used by the iterator to provide a fail-fast iterator.
	 * Any operation that changes the size of this collection should add 1 to
	 ...
	 * <p>
	 * Returns <code>true</code> if the collection is changed as a result of the
	 * call.
	 *
	 * @param element
	 *            the element whose presence in this collection is to be
	 *            ensured.
	 * @return <code>true</code> if the collection changed as a result of the
	 *         call.
	 * @throws IllegalArgumentException
	 ...
	 *         call (this can only be true if there was at least one instance of
	 *         <code>element</code> in this collection).
	 * @throws IllegalArgumentException
	 ...
	 *
	 * The following is true of the clone and the original: <blockquote>
	 * 
	 * <pre>
	 * x.clone() != x
	...
	public Object clone() {
		...
		return c;
		/*
		 * // the following produces a shallow clone of the collection Object
		 * result = null; try { result = super.clone(); }catch(
		 * CloneNotSupportedException e ) { // shouldn't get here, so don't do
		 * anything } return result;
		 */
	}
	...
	private class BasicIterator implements Iterator<E> {
		/**
		 * cursor is used to access the next element in the sequence. In its
		 * implementation, cursor is always the index of the next element to
		 * return. So cursor should be incremented _after_ retrieving the next
		 ...
		 * been preceded by a call to next() (they are paired). So if there has
		 * been NO call to next() prior to a remove() or if there were two
		 * remove() calls without an intervening next() call, it is NOT ok to
		...
		/**
		 * Determine if there are more elements in the iteration sequence.
		 * 
		 * @returns boolean <code>true</code> if there are more elements in the
		 *          iteration sequence.
		 */
		 ...
		 *             if this collection has been modified by a method outside
		 *             of this iterator.
		...
		/**
		 * remove the element returned by the last call to <code>next()</code>.
		 * 
		 * @throws ConcurrentModificationException
		 *             if this collection has been modified by a method outside
		 *             of this iterator.
		 * @throws IllegalStateException
		 *             if there has been no call to next() for this iteration or
		 *             if two calls to remove() have been made with no
