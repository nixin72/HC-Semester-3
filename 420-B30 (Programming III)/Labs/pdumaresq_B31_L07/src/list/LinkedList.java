...
//		DLNode<E> target = head.getSuccessor();
//		DLNode<E> lastUsed = head.getSuccessor();
		DLNode<E> newHead = tail.getPredecessor();
		DLNode<E> newTail = head.getSuccessor();
		
		newHead.setSuccessor(newHead.getPredecessor());
		newTail.setPredecessor(newTail.getSuccessor());
...
//			DLNode<E> current = target.getSuccessor();
//			head.setSuccessor(current.getSuccessor());
//			target.setSuccessor(current.getSuccessor());
//			current.getSuccessor().setPredecessor(current.getPredecessor());
//			current.setSuccessor(lastUsed);
//			current.setPredecessor(head);
//			lastUsed.setPredecessor(current);
//			lastUsed = current;
//		}
		
		for (int w = 0 ; w < size ; w++) {
		//	System.out.print(get(w) + " ");
		}
	}
	 ...
	 * listIterator() which returns a ListIterator object which will begin its
	 * iteration with the first element in the list. This is easily implemented
	 * as return listIterator(0);
	 * 
	 ...
	 * that all the indexed ("random access") operations are written on top of
	 * the list iterator. For example, add(index, element) can easily be
	 * implemented as ListIterator iter = new ListIterator(index); iter.add(
	 * element );
	 * 
	 * Implementations of remove(index), set(index), addAll(index, collection)
	 * can easily be implemented in a similar fashion. But how can there be
	 * concrete implementations of the optional methods defined in ASL? If these
	 * methods are not needed by a List implementation, they should throw an
	 * UnsupportedExceptionOperation. The answer is rather neat. By building the
	 * optional methods on top of the ListIterator methods, we can defer
	 * throwing an exception to ListIterator. If, on the other hand, we want to
	 * support an optional method, all we need to do is to provide the necessary
	 ...
	 * operations are not supported for a particular list implementation, all
	 * you need to do is implement ListIterator.add(element) to throw an
	 ...
	 * supported, ListIterator.add(element) is implemented accordingly (as we
	 * will do).
	 * 
	 * To support checks for concurrent modification, AL defines a modCount data
	 * field to keep track of all structural modifications to the list. This
	 * field is marked as protected in AL, which means that it is not available
	 * outide of the class, but it is available to subclasses of AL. Each time
	 * the list is structurally modified, the modCount field must be
	 * incremented. As before, when an Iterator or a ListIterator object is
	 * created, it must make a copy of the modCount field. If the
	 * expectedModCount value is different from modCount when any Iterator or
	 * ListIterator method is called, a ConcurrentModificationException must be
	 * thrown. The updating of the modCount field must be done by the add() and
	 * remove() methods from ListIterator because the add() and remove() methods
	 * defined in ASL do not do this.
	 * 
	 * While having the abstract classes available is a real help, it also
	 * incurs a responsibility to check the contracts for those classes to make
	 * sure that the methods we implement in LinkedList meet all the obligations
	 * laid out by an ancestor class. For example,
	 */
	...
	public class LinkedListIterator<E> implements ListIterator<E> {
		 ...
		 * been preceded by a call to next()/previous() (they are paired). So if
		 * there has been NO call to next()/previous() prior to a remove() or if
		 * there were two remove() calls without an intervening
		 * next()/previous() call, it is NOT ok to try to remove an item.
		 */
		...
		public void remove() {
			...
			if (cursor == lastNodeReturned) // removing item returned by a
											// previous() call
